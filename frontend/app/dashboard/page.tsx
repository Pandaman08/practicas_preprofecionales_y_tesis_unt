...
function getDeltaPercent(current: number, base: number) {
...
function BasicRoleDashboard({ resumen }: { resumen?: DashboardResumen }) {
  ...
  const primaryKpis = rankedKpis.slice(0, 3);
  const secondaryKpis = rankedKpis.slice(3);
  const maxKpiValue = rankedKpis.length ? Math.max(...rankedKpis.map((item) => item.value)) : 0;
  const totalKpiValue = rankedKpis.reduce((acc, item) => acc + item.value, 0);
  const averageKpiValue = rankedKpis.length ? Math.round(totalKpiValue / rankedKpis.length) : 0;

  const strongestKpi = rankedKpis[0];
  const weakestKpi = [...rankedKpis].sort((a, b) => a.value - b.value)[0];
  const underAverageCount = rankedKpis.filter((item) => item.value < averageKpiValue).length;

  const autoInsights = [
    trend
      ? `${trend.label} ${trend.delta >= 0 ? 'aumento' : 'disminuyo'} ${Math.abs(trend.percent)}% respecto al periodo anterior.`
      : 'No hay variacion interperiodo disponible para este rol.',
    strongestKpi
      ? `${strongestKpi.name} lidera el periodo con ${strongestKpi.value} registros.`
      : 'No hay KPI dominante en este periodo.',
    weakestKpi
      ? `${weakestKpi.name} se mantiene por debajo del promedio del panel (${averageKpiValue}).`
      : 'No hay KPI por debajo del promedio.',
    underAverageCount
      ? `${underAverageCount} indicadores estan por debajo del promedio y requieren seguimiento.`
      : 'Todos los indicadores estan en o sobre el promedio del panel.',
  ].slice(0, 3);
    ...
    <div className="space-y-4">
      <section className="grid grid-cols-1 items-start gap-4 md:[grid-template-columns:repeat(auto-fit,minmax(280px,1fr))] 2xl:[grid-template-columns:repeat(auto-fit,minmax(340px,1fr))]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600">{resumen?.role}</span>
          </div>
          ...
          {trend ? (
            <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-2 flex items-end justify-between gap-3">
                <div>
                  <p className="text-4xl font-bold tracking-tight text-slate-900">{trend.current}</p>
          ...
          <div className="mt-4 space-y-2">
            {autoInsights.map((message, idx) => (
              <div key={`${message}-${idx}`} className="rounded-lg border border-blue-100 bg-blue-50/60 px-3 py-2 text-xs text-blue-900">
                {message}
              </div>
            ))}
          </div>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-slate-800">KPIs clave del rol</h3>
            <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600">Top {primaryKpis.length}</span>
          </div>

          <div className="grid grid-cols-1 gap-3 sm:[grid-template-columns:repeat(auto-fit,minmax(170px,1fr))] xl:[grid-template-columns:repeat(auto-fit,minmax(210px,1fr))]">
            {primaryKpis.map((item) => {
              const deltaVsAverage = getDeltaPercent(item.value, averageKpiValue);
              const tone = getDeltaTone(deltaVsAverage);
              return (
                <div key={`primary-${item.key}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">{item.name}</p>
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    >
                      {tone === 'up' ? '↑' : tone === 'down' ? '↓' : '•'} {Math.abs(deltaVsAverage)}%
                    </span>
                    <p className="text-[11px] text-slate-500">vs promedio del panel</p>
                  </div>
                  {item.hint ? <p className="mt-2 text-[11px] text-slate-500">{item.hint}</p> : null}
                </div>
              );
            })}
          </div>
        ...
        <article className="h-full rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-3 grid grid-cols-1 gap-2 text-xs text-slate-600 sm:grid-cols-2">
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              <p className="font-semibold text-slate-800">Maximo del periodo</p>
              <p>{strongestKpi ? `${strongestKpi.name}: ${strongestKpi.value}` : 'Sin datos'}</p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              <p className="font-semibold text-slate-800">Promedio del panel</p>
              <p>{averageKpiValue}</p>
            </div>
          </div>
        </article>
            ...
            {rankedKpis.length ? (
              <div className="min-h-0 flex-1 space-y-2.5 overflow-auto pr-1">
                {rankedKpis.map((item) => {
                  ...
                  return (
                    <div key={`summary-${item.key}`} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600">Contexto automatico</span>
        </div>

        {highlights.length ? (
          <div className="grid grid-cols-1 gap-2 sm:[grid-template-columns:repeat(auto-fit,minmax(200px,1fr))] xl:[grid-template-columns:repeat(auto-fit,minmax(240px,1fr))]">
